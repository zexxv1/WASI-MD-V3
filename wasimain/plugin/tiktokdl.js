...
const tiktokCommand = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      ...
      console.error("WASI API NOT RESPONDING :", error);
      await m.reply('WASI API NOT RESPONDING.');
      await m.React("❌");
    }
  } else if (selectedId) { 
      ...
      if (selectedMedia) {
        try {
          ...
          if (type === 'video' && fileSizeInMB <= 300) {
            content = { video: finalMediaBuffer, mimetype: 'video/mp4', caption: '> FILE GENERATED BY WASI MD V3�' };
          } else if (type === 'audio' && fileSizeInMB <= 300) {
            content = { audio: finalMediaBuffer, mimetype: 'audio/mpeg', caption: '> FILE GENERATED BY WASI MD V3�' };
          }
